...
function headUi(){
	var $menu = $('#gnb'),
		$wrap = $('.gnb_wrap'),
		$length = $menu.find('.swiper-slide').length;
		$idx = $('#gnb .on').index(),
		$btnPrev = $('.gnb_navi .ui-prev'),
		$btnNext = $('.gnb_navi .ui-next');

	subSwiper = $menu.swiper({
		slidesPerView:'auto',
		...
		onFirstInit:function(swiper){
			...
			$menu.find('.swiper-wrapper').css('width','+=2');
	...
	$('#gnb li a').click(function(e) {
		...
		var $href = $(this).attr('href'),
			$gnbH =$('.gnb_wrap').outerHeight();
			
		if($($href).length > 0){
			
			var $secTop =$($href).offset().top,
				$top = $secTop - $gnbH;
			console.log($secTop,$gnbH)
			$('html,body').stop().animate({'scrollTop':$top});
		}else{
			console.log('해당위치가 없음')
		}
	});

	$('.section:odd').addClass('even');
	scrollOn('.section', '#gnb .swiper-slide', 200);
}
...
function scrollOn(tar, navi, range){
	$(window).bind('load scroll resize',function(){
		...
		var $onIdx = $(navi).siblings('.on').first().index();
		
		subSwiper.swipeTo($onIdx);
	});	
}
